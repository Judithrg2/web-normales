(function () {
  ...
  if (toggle && nav) {
    ...
    window.addEventListener('resize', function () {
      if (!window.matchMedia('(max-width: ' + MOBILE_BP + 'px)').matches) closeMenu();
    });
  }
  ...
  if (form) {
    ...
    var curso = form.querySelector('#curso');
    var privacidad = form.querySelector('#privacidad');

    [nombre, telefono, curso].forEach(function (el) {
      if (!el) return;
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (privacidad && !privacidad.checked) {
        privacidad.setAttribute('aria-invalid', 'true');
        valid = false;
      }
